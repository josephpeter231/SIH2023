...
const Engine = React.memo(() => {
  ...
  const predictivePdf = () => {
    const pdfFilePath = process.env.PUBLIC_URL + '/Rubbing.pdf';

    // Create a download link
    const link = document.createElement('a');
    link.href = pdfFilePath;
    link.download = 'Analysis Report Crankshaft Bearing 1.pdf';
  ...
  const config = {
    labels: goodBearingData.map((data) => data.time),
    datasets: [
      {
        label: "Hertz",
        data: goodBearingData.map((data) => data.value),
        backgroundColor: [
          "#4cceac",
        ],
        borderColor: "#4cceac",
  ...
  const configFft = {
    labels: fftGoodBearingData.map((data) => data.time),
    datasets: [
      {
        label: "Hertz",
        data: fftGoodBearingData.map((data) => data.value),
        backgroundColor: [
          "#4cceac",
        ],
        borderColor: "#4cceac",
  ...
  return (
    <Box m="20px" marginTop={0} className="home">
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Header title="Engine Crankshaft Bearing - 1" subtitle="Know your vehicle health" />
